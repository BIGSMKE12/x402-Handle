...
import { TrustScoreBreakdown } from "@/components/stellar/TrustScoreBreakdown";
import { getStellarProviderIntelligence } from "@/lib/api/client";

// Ruta para providers Stellar (id = "<contractId>/<providerId>", siempre con "/").
// Las otras sub-rutas de este [providerId] (customers, api-growth, etc.) son
// del catálogo multi-chain legacy y usan ids sin "/" — no colisionan.
export default async function StellarProviderDetailPage({
  ...
  const intelligence = await getStellarProviderIntelligence(providerId);
