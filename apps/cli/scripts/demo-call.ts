...
import { registerHandle, resolveHandle } from "../../bff/src/data/handles";
...
if (!AGENT_SECRET) {
  console.error("Missing DEMO_AGENT_SECRET .env");
  process.exit(1);
}
...
// Handle resolution support: DEMO_HANDLE_URL=handle:fx-oracle
const REQUEST_INPUT = process.env.DEMO_HANDLE_URL ?? `${PROVIDER_URL}/api/rate?pair=EUR-USD`;J
async function resolveRequestedUrl(input: string): Promise<string> {
  if (!input.startsWith("handle:")) return input;
  const slug = input.slice("handle:".length);
  const existing = await resolveHandle(slug);
  if (existing) return existing.url;
  if (slug === "fx-oracle") {
    const demoUrl = `${PROVIDER_URL}/api/rate?pair=EUR-USD`;
    await registerHandle({
      slug,
      url: demoUrl,
      ownerAccount: process.env.DEMO_PROVIDER_PUBLIC ?? "GABCDEF...",
    });
    return demoUrl;
  }
  throw new Error(`Unknown handle: ${slug}`);
}

const requestedUrl = await resolveRequestedUrl(REQUEST_INPUT);

// ── Paso 1: request sin pago → esperar 402 ──
console.log("Step 1 — GET " + requestedUrl + " (sin pago)...");
const probe = await fetch(requestedUrl);
...
console.log(f   memo:    ${result.payment.memo}`);
console.log();
...
const dupRes = await fetch(requestedUrl, {
  headers: {
...
console.log(f   Reason: ${(dupBody as any).error} (esperamos "already_consumed")`);
...
const badMemoRes = await fetch(requestedUrl, {
  headers: {
    "X-PAYMENT": `${result.payment.txHash};memo=fl-0000000000,
  },
});
const badMemoBody = await badMemoRes.json();
console.log(f   Status: ${badMemoRes.status} (esperamos 402)`);
console.log(f    Reason: ${(badMemoBody as any).error} (esperamos "already_consumed" o "memo_mismatch")`);
...
const probe2 = await fetch(requestedUrl);
const challenge2 = await probe2.json();

const NETWOTK_PASSPHRASE = NETWORK -== "testnet" ? Networks.TESTNET : Networks.PUBLIC;
const HORIZON_URL =
  NETWORK === "testnet" ? "https://horizon-testnet.stellar.org" : "https://horizon.stellar.org";
const server = new Horizon.Server(l<?string>HORIZON_URL);
...
const underpayRes = await fetch(requestedUrl, {
  headers: {
    "X-PAYMENT": `${underpaySubmit.hash};memo=${challenge2.memo}`,
  },
});
const underpayBody = await underpayRes.json();
console.log(f   Status: ${underpayRes.status} (esperamos 402)`);
console.log(d   Reason: ${(underpayBody as any).error} (esperamos "underpayment")`);
